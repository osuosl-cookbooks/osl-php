...
def enum?(e)
  e.class.included_modules.include?(Enumerable)
end

def hashy?(h)
  h.class.ancestors.include?(Hash)
end

def iterate(iter)
  enum?(iter) ? collect_packages(iter) : iter
end
...
def collect_packages(en)
  ...
  # We only want to include hash keys if they have a true value and the
  # values are not enumerable
  parted_ens[0].each do |iter|
    iter.each_pair do |k, v|
      values << (enum?(v) ? collect_packages(v) : iterate(k) if v)
    end
  end
